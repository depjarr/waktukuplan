'use client';
import { fmtDate, isPastEvent, pad } from '@/lib/dates';
import { useNow } from '@/hooks/useNow';
import { cmpEvent, usePlanner } from './PlannerProvider';

/** Jadwal bertanda ★ pada bulan yang tampil di kalender mini.
 *  Otomatis kecoret kalau jamnya sudah lewat, walau belum ditandai selesai manual. */
  ...
  return (
    <div className="card">
      ...
      <ul className="rem">
        ...
        {list.map((e) => {
          const passed = !e.done && !!now && isPastEvent(e, now);
          return (
            <li key={e.id} className="rrow">
              ...
              <button className="sbody" onClick={() => openEvent({ id: e.id })}>
                <span className="stitle" style={(e.done || passed) ? { textDecoration: 'line-through', opacity: 0.6 } : undefined}>{e.title}</span>
              </button>
